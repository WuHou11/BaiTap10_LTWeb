...
public class ProductAPIController {
	...
	public ResponseEntity<?> getAllProducts() {
		return new ResponseEntity<>(new Response(true, "Danh sách sản phẩm", productService.findAll()), HttpStatus.OK);
	}

	@PostMapping("/getProduct")
	public ResponseEntity<?> getProduct(@Validated @RequestParam("id") Long id) {
		Optional<Product> opt = productService.findById(id);
		return opt.map(p -> new ResponseEntity<>(new Response(true, "Thành công", p), HttpStatus.OK)).orElse(
				new ResponseEntity<>(new Response(false, "Không tìm thấy sản phẩm", null), HttpStatus.NOT_FOUND));
	}
	...
	public ResponseEntity<?> addProduct(@Valid @ModelAttribute ProductModel productModel, // <-- dùng @Valid
	...
	public ResponseEntity<?> updateProduct(@Valid @ModelAttribute ProductModel productModel,
			BindingResult bindingResult, @RequestParam(value = "image", required = false) MultipartFile image) {
		...
		Optional<Product> optProduct = productService.findById(productModel.getProductId());
